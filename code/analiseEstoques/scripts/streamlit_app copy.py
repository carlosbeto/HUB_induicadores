...
def load_eficiencia_bu_mast(db_path_str: str, mes_ref: str) -> pd.DataFrame:
    """
    Eficiência do baseline segmentada por BU (area_negocio).

    Regras:
    - baseline: snapshot baseline_mensal do MAST
    - saídas: MB51 H no mês
    - classificação: dim_material_grupo.area_negocio
    """
    ...
    with connect_readonly(db_path) as con:
        ...
        # ---------------------------------------------------------
        # Descobre a data baseline do mês
        # ---------------------------------------------------------
        ...
        df_mov = pd.read_sql_query("""
            SELECT
                ...
                SUM(
                    CASE
                        WHEN fm.deb_cred = 'S'
                         AND TRIM(COALESCE(fm.ordem, '')) = ''
                        THEN COALESCE(fm.valor_estimado, 0)
                        ELSE 0
                    END
                ) AS entradas_val,
            ...
            FROM fact_mb51_mov fm

            LEFT JOIN dim_material_grupo dmg
                ON fm.material = dmg.material

            WHERE fm.deposito = 'MAST'
              AND fm.data_lancamento >= ?
              AND fm.data_lancamento < ?
    ...
    df["baseline_val"] = (
        pd.to_numeric(df["baseline_val"], errors="coerce")
        .fillna(0.0)
    )

    df["saidas_val"] = (
        pd.to_numeric(df["saidas_val"], errors="coerce")
        .fillna(0.0)
    )

    df["entradas_val"] = (
        pd.to_numeric(df["entradas_val"], errors="coerce")
        .fillna(0.0)
    )

    # -------------------------------------------------------------
    # Novo conceito gerencial:
    # eficiência operacional = saídas / (baseline inicial + entradas)
    #
    # Isso considera:
    # - o estoque que já existia no início do mês
    # - mais tudo que entrou durante o mês
    # - contra tudo que saiu durante o mês
    # -------------------------------------------------------------

    df["base_disponivel_val"] = df["baseline_val"] + df["entradas_val"]

    df["saldo_operacional_val"] = (
        df["base_disponivel_val"] - df["saidas_val"]
    ).clip(lower=0)

    df["eficiencia_operacional_pct"] = np.where(
        df["base_disponivel_val"] > 0,
        df["saidas_val"] / df["base_disponivel_val"],
        0
    )

    # Mantém compatibilidade com os gráficos já criados.
    # Agora "eficiencia_pct" passa a representar a eficiência operacional completa.
    df["eficiencia_pct"] = df["eficiencia_operacional_pct"]

    # Mantém nome antigo para não quebrar a tabela atual.
    df["baseline_restante"] = df["saldo_operacional_val"]

    total_saidas = float(df["saidas_val"].sum())

    df["participacao_saida_pct"] = np.where(
        total_saidas > 0,
        df["saidas_val"] / total_saidas,
        0
    )
    ...
    # participação no baseline total
    df["participacao_baseline_pct"] = np.where(
        total_baseline > 0,
        df["baseline_val"] / total_baseline,
        0
    ...
    # Regra inicial: BU com menos de 3% do baseline total é "baixo impacto".
    df["classe_relevancia"] = np.where(
        df["participacao_baseline_pct"] >= 0.03,
        "Relevante",
        "Baixo impacto"
...
def run():
    ...
    with tab1:
        ...
        if df_bu.empty:
            ...
            df_bu_view["Saídas no mês (R$)"] = df_bu_view["saidas_val"].map(lambda x: _fmt_ptbr_num(x, 2))
            df_bu_view["Baseline restante (R$)"] = df_bu_view["baseline_restante"].map(lambda x: _fmt_ptbr_num(x, 2))
            ...
            df_bu_view["Entradas no mês (R$)"] = df_bu_view["entradas_val"].map(lambda x: _fmt_ptbr_num(x, 2))
            df_bu_view["Base disponível (R$)"] = df_bu_view["base_disponivel_val"].map(lambda x: _fmt_ptbr_num(x, 2))
            df_bu_view["Saldo operacional (R$)"] = df_bu_view["saldo_operacional_val"].map(lambda x: _fmt_ptbr_num(x, 2))
    ...
    with tab3:
        ...
        cols_show = [
            ...
            "familia", "subfamilia", "processamento", "linha_unid_negocio"
        ]
